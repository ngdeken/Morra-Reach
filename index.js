...
const intToOutcome = ['Beta wins!', 'Draw!', 'Adam wins!'];
...
class Player extends React.Component {
  ...
  async seeSum() { // Fun([], UInt)
    const sum = await new Promise(resolveSumP => {
      this.setState({view: 'GetGuess', playable: true, resolveSumP});
    });
    this.setState({view: 'WaitingForResults', sum});
    return sum;
  }
  seeOutcome(i) { this.setState({view: 'Done', outcome: intToOutcome[i]}); }
  informTimeout() { this.setState({view: 'Timeout'}); }
  seeSum(sum) { this.setState({view: 'GetSum', sum: GUESS[sum]}) }; 
